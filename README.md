...
1. Create a new Google Sheet. Add a header row to the first tab, e.g.:
   ...
   Timestamp is written as `dd-mm HH:MM IST` (the trailing "IST" is deliberate — without it Sheets auto-converts the text into its internal date-serial number, which then displays as a raw number like `46062.94049` depending on the cell's format), DOB as `dd-mm-yyyy`, and Age is calculated from DOB automatically — none of these need to be typed in by hand.
